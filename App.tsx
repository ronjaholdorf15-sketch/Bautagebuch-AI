  ...
  const nextcloudProxy = {
    async exists(url: string, creds: { user: string, pass: string }) {
      try {
        ...
        const response = await fetch('/nc-bridge-v3', {
          method: 'POST',
        ...
        if (response.status === 405) {
          const getResponse = await fetch('/nc-bridge-v3', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
                                            ...
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    try {
                                                        ...
                                                        const pingRes = await fetch('/nc-ping-v3');
                                                        const aisServer = pingRes.headers.get('X-AIS-Server');
                                                        const aisTime = pingRes.headers.get('X-AIS-Timestamp');
                                                        log += `Status: ${pingRes.status} [AIS-Server: ${aisServer || 'NICHT GEFUNDEN'}] [Time: ${aisTime || '?'}]\n`;
                                                        
                                                        if (pingRes.status === 200) {
                                                            try {
                                                                const pingData = await pingRes.json();
                                                                log += `Server-Info: ${pingData.version} (Node: ${pingData.node}) (Env: ${pingData.env})\n`;
                                                            } catch (e) {
                                                    ...
                                                    for (const sUrl of statusUrls) {
                                                        try {
                                                            ...
                                                            const sRes = await fetch('/nc-bridge-v3', {
                                                                method: 'POST',
                                                                headers: { 'Content-Type': 'application/json' },
                                                    ...
                                                    for (const url of variations) {
                                                        ...
                                                        try {
                                                            // Try OPTIONS first
                                                            const optRes = await fetch('/nc-bridge-v3', {
                                                                method: 'POST',
                                                                headers: { 'Content-Type': 'application/json' },
                                                            ...
                                                            let res = await fetch('/nc-bridge-v3', {
                                                                method: 'POST',
                                                                headers: { 'Content-Type': 'application/json' },
                                                                body: JSON.stringify({ 
                                                                    url, 
